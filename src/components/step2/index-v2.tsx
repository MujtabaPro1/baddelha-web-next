...
const Step2 = () => {
    ...
    const [trims, setTrims] = useState<{id: number; name: string}[]>([]);
    ...
    // Fetch vehicle specs from new API
    const fetchVehicleSpecs = async (make: string, model: string, year: string) => {
        try {
            console.log('Fetching vehicle specs from new API...');
            const response = await axiosInstance.get('/api/1.0/car/estimation/vehicle-specs', {
        ...
        axiosInstance.post(`/api/1.0/car/specs`,{
            ...
            .then((response) => {
                ...
                if(data.trims && Array.isArray(data.trims) && data.trims.length > 0){
                    ...
                    console.log('Applied trims from fallback API:', uniqueTrims.length, 'trims available');
                }
            })
            .catch((error) => {
                ...
                <form onSubmit={handleSubmit} className="p-4 md:p-6">
                    ...
                    <div className="space-y-4 mb-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="relative">
                                    ...
                                    >
                                        {loading.bodyTypes ? (
                                            ...
                                            bodyTypes.map((type) => (
                                                <option key={String(type.id) || 'BodyType_' + Math.random()} value={String(type.id)}>{type.name}</option>
                                            ))
                                        ) : (
                            ...
                            {/* Trim - Only shown when trims are available */}
                            {trims.length > 0 && (
                                <div className="bg-slate-50 rounded-xl p-4">
                                    <label className="block text-xs font-medium text-slate-500 mb-2">{language === 'en' ? 'Trim' : 'الفئة'}</label>
                                    <div className="relative">
                                        <select
                                            value={trim}
                                            onChange={(e) => {
                                                setTrim(e.target.value);
                                                setTrimName(trims.find(t => String(t.id) === e.target.value)?.name || '');
                                            }}
                                            className="block w-full rounded-xl border-0 bg-white py-3 px-4 pr-10 ring-1 ring-inset ring-slate-200 focus:ring-2 focus:ring-slate-900 appearance-none font-medium text-gray-900"
                                        >
                                            {trims.map(t => (
